/*
 * Three-Leg Stance with Body Shift
 *
 * Default + 다리 들기 전에 본체를 들 다리 반대 방향으로 shift.
 * 무게 중심을 지지 삼각형 안에 위치시켜 정적 안정 확보.
 *
 * Default pose v4 Case A2: foot_x = -10, foot_z = -130 (각 hip frame)
 *
 * Shift 방향 (앞뒤만, 일단):
 *   FR/FL (앞 다리) 들기 → 발들 +x (앞으로) 30mm 이동 (본체 뒤로)
 *   RL/RR (뒤 다리) 들기 → 발들 -x (뒤로) 30mm 이동 (본체 앞으로)
 *
 * IK:
 *   foot_x, foot_z (hip frame) → thigh, knee raw offset
 *   sign[]으로 좌우 모터 처리
 *
 * 각 trial:
 *   default → shift_pose → lift_pose → shift_pose → default
 *
...
/* ===== Default (foot 기준, hip frame) ===== */
#define DEFAULT_FOOT_X    -10.0f
#define DEFAULT_FOOT_Z   -130.0f

/* ===== Body Shift ===== */
#define BODY_SHIFT_X       30.0f   /* mm, 앞뒤 shift 양 */

/* ===== 들기 ===== */
#define LIFT_KNEE_DEG      30
...
#define LIFT_SPEED_DEG_PER_SEC  15
#define STEP_MS                 25
...
#define TRANS_SETTLE_MS         1500    /* shift/unshift 후 안정화 */
#define LIFT_SETTLE_MS          1500
#define LIFT_MEASURE_MS         3000
...
/* ============================================================ */
/* IK — foot (x, z) hip frame → thigh, knee raw offset          */
/* ============================================================ */

/* foot_x, foot_z (hip frame) → thigh raw offset, knee raw offset (sign 미적용) */
...
/* 모든 다리에 동일한 (foot_x, foot_z) 적용 */
static void compute_pose_uniform_foot(pose_t pose, float foot_x, float foot_z) {
    int thigh_off, knee_off;
    ik_to_raw_offsets(foot_x, foot_z, &thigh_off, &knee_off);

    for (int l = 0; l < NUM_LEGS; l++) {
        int8_t s = legs[l].sign;
        pose[l][0] = ZERO_POS + s * HIP_OFFSET;
        pose[l][1] = (uint16_t)(ZERO_POS + s * thigh_off);
        pose[l][2] = (uint16_t)(ZERO_POS + s * knee_off);
    }
}

/* lift_idx 다리만 knee +30° 추가 굽힘 */
static void apply_lift_to_pose(pose_t pose, int lift_idx) {
    int8_t s = legs[lift_idx].sign;
    pose[lift_idx][2] = (uint16_t)((int)pose[lift_idx][2] + s * LIFT_KNEE_UNIT);
}

/* shift 방향: 앞 다리 들기 → 발들 +x, 뒤 다리 들기 → 발들 -x */
static float shift_foot_x_for_lift(int lift_idx) {
    if (lift_idx == IDX_FR || lift_idx == IDX_FL) {
        return DEFAULT_FOOT_X + BODY_SHIFT_X;   /* 발들 앞으로 */
    } else {
        return DEFAULT_FOOT_X - BODY_SHIFT_X;   /* 발들 뒤로 */
    }
}

static const char* shift_direction_str(int lift_idx) {
    if (lift_idx == IDX_FR || lift_idx == IDX_FL) {
        return "body BACKWARD (foot_x +30)";
    } else {
        return "body FORWARD (foot_x -30)";
    }
}
...
static void measure_trial(trial_result_t* result, uint32_t duration_ms) {
    ...
    for (int s = 0; s < n_samples; s++) {
        ...
        for (int l = 0; l < NUM_LEGS; l++) {
            ...
            if (r.ok) knee_sum[l] += (r.load & 0x3FF);
        }
        ...
        if (bno_read_body(&b)) {
            ...
            if (!safety_check(b.pitch, b.roll)) {
                result->aborted = true;
                g_abort = true;
                printf("    *** ABORT: tilt exceeded (pitch=%+.1f roll=%+.1f) ***\r\n",
                       b.pitch, b.roll);
                return;
            }
        }

        printf("      s%d: pitch=%+.2f roll=%+.2f knee[FL=%4u FR=%4u RL=%4u RR=%4u]\r\n",
               s + 1,
...
static void run_baseline_trial(trial_result_t* r) {
    ...
    printf("\r\n=================================================\r\n");
    printf(" BASELINE (4-leg standing, no shift)\r\n");
    printf("=================================================\r\n");
    measure_trial(r, BASELINE_MEASURE_MS);

    printf("\r\n    pitch=%+.2f roll=%+.2f  knee[FL=%d FR=%d RL=%d RR=%d]\r\n",
           r->pitch_avg, r->roll_avg,
           r->knee_load[IDX_FL], r->knee_load[IDX_FR],
           r->knee_load[IDX_RL], r->knee_load[IDX_RR]);
}

static void run_shift_lift_trial(trial_result_t* r, int lift_idx,
                                  const pose_t default_pose) {
    r->label = legs[lift_idx].leg_name;
    r->lifted_idx = lift_idx;
    r->aborted = false;

    float shift_x = shift_foot_x_for_lift(lift_idx);

    printf("\r\n=================================================\r\n");
    printf(" LIFT %s with body shift\r\n", legs[lift_idx].leg_name);
    printf("    Direction: %s\r\n", shift_direction_str(lift_idx));
    printf("    Target foot_x = %+.0f mm\r\n", shift_x);
    printf("=================================================\r\n");

    /* 1. Build shift pose */
    pose_t shift_pose;
    compute_pose_uniform_foot(shift_pose, shift_x, DEFAULT_FOOT_Z);

    /* 2. Build lift pose (shift + lift) */
    pose_t lift_pose;
    memcpy(lift_pose, shift_pose, sizeof(pose_t));
    apply_lift_to_pose(lift_pose, lift_idx);

    /* 3. default → shift */
    printf("    Phase 1: body shift...\r\n");
    transition_all(default_pose, shift_pose, SPEED_DEG_PER_SEC);
    delay_with_estop(TRANS_SETTLE_MS);
    if (g_abort) return;

    /* 4. shift → lift */
    printf("    Phase 2: lift %s...\r\n", legs[lift_idx].leg_name);
    transition_all(shift_pose, lift_pose, LIFT_SPEED_DEG_PER_SEC);
    delay_with_estop(LIFT_SETTLE_MS);

    /* 5. measure */
    printf("    Phase 3: measuring %d ms...\r\n", LIFT_MEASURE_MS);
    measure_trial(r, LIFT_MEASURE_MS);

    /* 6. lift → shift (다리 내림) */
    if (!r->aborted) {
        printf("    Phase 4: lower %s...\r\n", legs[lift_idx].leg_name);
        transition_all(lift_pose, shift_pose, LIFT_SPEED_DEG_PER_SEC);
        delay_with_estop(TRANS_SETTLE_MS);
    } else {
        printf("    *** ABORTED, returning fast ***\r\n");
        transition_all(lift_pose, shift_pose, SPEED_DEG_PER_SEC);
    }

    /* 7. shift → default (본체 원위치) */
    printf("    Phase 5: unshift body...\r\n");
    transition_all(shift_pose, default_pose, SPEED_DEG_PER_SEC);
    delay_with_estop(TRANS_SETTLE_MS);
...
static void print_summary(void) {
    printf("\r\n");
    printf("=================================================\r\n");
    printf("  THREE-LEG WITH BODY SHIFT — SUMMARY\r\n");
    printf("=================================================\r\n");
    printf("\r\n");
    ...
    for (int i = 0; i < NUM_TRIALS; i++) {
        ...
        printf("  %-8s | %+6.2f   %+6.2f   | %3d   %3d   %3d   %3d  | %s\r\n",
               t->label, t->pitch_avg, t->roll_avg,
               t->knee_load[IDX_FL], t->knee_load[IDX_FR],
               t->knee_load[IDX_RL], t->knee_load[IDX_RR],
               (t->lifted_idx >= 0) ? legs[t->lifted_idx].leg_name : "---");
    }

    if (trials[0].aborted) return;

    printf("\r\n  Stability (Δtilt vs baseline):\r\n");
    for (int i = 1; i < NUM_TRIALS; i++) {
        trial_result_t* t = &trials[i];
        if (t->aborted) continue;
        float dp = t->pitch_avg - trials[0].pitch_avg;
        float dr = t->roll_avg  - trials[0].roll_avg;
        float total = sqrtf(dp*dp + dr*dr);
        const char* eval;
        if (total < 3.0f) eval = "STABLE  ";
        else if (total < 10.0f) eval = "MARGINAL";
        else eval = "UNSTABLE";
        printf("    Lift %s: Δpitch=%+5.2f Δroll=%+5.2f total=%5.2f [%s]\r\n",
               t->label, dp, dr, total, eval);
    }
    printf("=================================================\r\n");
}
...
int main(void) {
    ...
    printf("\r\n=== STM32F446RE Boot ===\r\n");
    printf("\r\n=================================================\r\n");
    printf("  Three-Leg with Body Shift Test\r\n");
    printf("  Default foot: x=%.0f, z=%.0f\r\n", DEFAULT_FOOT_X, DEFAULT_FOOT_Z);
    printf("  Body shift:   %.0f mm  (foot_x ±30)\r\n", BODY_SHIFT_X);
    ...
    {
        int t1, k1, t2, k2;
        ik_to_raw_offsets(DEFAULT_FOOT_X + BODY_SHIFT_X, DEFAULT_FOOT_Z, &t1, &k1);
        ik_to_raw_offsets(DEFAULT_FOOT_X - BODY_SHIFT_X, DEFAULT_FOOT_Z, &t2, &k2);
        printf("\r\n  IK preview:\r\n");
        printf("    foot_x=+20: thigh=%d (%.1f) knee=%d (%.1f)\r\n",
               t1, t1*360.0f/4096, k1, k1*360.0f/4096);
        printf("    foot_x=-40: thigh=%d (%.1f) knee=%d (%.1f)\r\n",
               t2, t2*360.0f/4096, k2, k2*360.0f/4096);
    }
    ...
    pose_t default_pose;
    compute_pose_uniform_foot(default_pose, DEFAULT_FOOT_X, DEFAULT_FOOT_Z);
    ...
    /* === Each leg with body shift === */
    for (int i = 0; i < (int)NUM_LIFTS; i++) {
        if (g_abort) {
            printf("\r\n*** Global abort, skipping remaining lifts ***\r\n");
            trials[1 + i].aborted = true;
            trials[1 + i].label = legs[lift_order[i]].leg_name;
            continue;
        }
        run_shift_lift_trial(&trials[1 + i], lift_order[i], default_pose);
    }
