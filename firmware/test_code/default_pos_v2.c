/*
 * New Default Pose Baseline Measurement
 *
 * 새 default (FL 손으로 측정한 자세 기반):
 *   thigh offset = -774  (-68.0°)
 *   knee offset  = +1338 (+117.6°)
 *   foot_x ≈ +1.6 mm  (hip 거의 바로 아래)
 *   foot_z ≈ -123.6 mm
 *
 * 이전 default (-512, +1024) 대비 다리가 더 굽혀짐.
 * 본체가 낮은 자세, 발이 hip 바로 아래.
 *
 * 측정:
 *   - 4 다리 default standing
 *   - 5초 settling 후 10초 모니터링 (1초마다)
 *   - IMU body pitch/roll
 *   - 12 서보 load (Front:Rear, Left:Right 비율)
 *
 * IMUPLUS 모드 (자기 간섭 면역).
 * 정지: ESC
 */
...
/* ===== ★ NEW Default pose ★ ===== */
#define HIP_OFFSET           0
#define THIGH_OFFSET      (-774)    /* -68.0° (이전 -512) */
#define KNEE_OFFSET       (+1338)   /* +117.6° (이전 +1024) */
