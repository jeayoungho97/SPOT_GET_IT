...
class MapConfig:
    def __init__(self, path: str):
        ...
        self.field:       dict                 = cfg["field"]
        self.wp_sampling: dict                 = cfg["waypoint_sampling"]
        self.obstacles:   List[dict]           = cfg.get("obstacles", [])

        # Auto grid step: shorter side / 20, rounded to nearest 0.5 m, min 0.5 m
        _w   = self.field["x_max"] - self.field["x_min"]
        _h   = self.field["y_max"] - self.field["y_min"]
        _raw = min(_w, _h) / 20.0
        self.grid_step: float = max(1.0, round(_raw * 2) / 2)
...
# ── 격자 유틸 ─────────────────────────────────────────────
def _to_cell(x: float, y: float, x_min: float, y_min: float, grid_step: float = 1.0) -> Cell:
    return (math.floor((x - x_min) / grid_step), math.floor((y - y_min) / grid_step))

def _cell_center(cell: Cell, x_min: float, y_min: float, grid_step: float = 1.0) -> Tuple[float, float]:
    ...
    while x < r["x_max"]:
        ...
        while y < r["y_max"]:
            if not _pt_blocked(x, y, cfg, use_clearance=False):
                cells.add(_to_cell(x, y, x_min, y_min, gs))
            y += gs
        x += gs
...
def _cells_within_radius(waypoints: List[Waypoint], radius: float,
                          ...
                          grid_step: float = 1.0) -> FrozenSet[Cell]:
        ...
        for dc in range(-r_idx, r_idx + 1):
            for dr in range(-r_idx, r_idx + 1):
                ...
                if cell not in all_cells:
                    continue
                cx, cy = _cell_center(cell, x_min, y_min, grid_step)
...
def _build_wall_cells(cfg: MapConfig, all_cells: FrozenSet[Cell]) -> FrozenSet[Cell]:
    ...
    r_idx = int(WALL_RADIUS / gs) + 1
    for px, py in boundary_pts:
        bc = math.floor((px - x_min) / gs)
        br = math.floor((py - y_min) / gs)
        for dc in range(-r_idx, r_idx + 1):
            for dr in range(-r_idx, r_idx + 1):
                cell = (bc + dc, br + dr)
                if cell not in all_cells:
                    continue
                cx, cy = _cell_center(cell, x_min, y_min, gs)
                if (cx - px) ** 2 + (cy - py) ** 2 <= r2:
                    wall.add(cell)
...
def _build_paths_for(robot_key: str, start: Waypoint,
    ...
    grid_xs = [round(x, 2) for x in np.arange(cfg.field["x_min"] + gs,
                                                cfg.field["x_max"], gs)]
    grid_ys = [round(y, 2) for y in np.arange(cfg.field["y_min"] + gs,
                                                cfg.field["y_max"], gs)]

    diag_max = max(cfg.field["x_max"] - cfg.field["x_min"],
                   cfg.field["y_max"] - cfg.field["y_min"])
    ...
    while queue:
        ...
        if _path_valid([curr, end], cfg):
            ...
            if len(final_wps) <= MAX_WAYPOINTS:
                ...
                if w_tuple not in seen_wps:
                    ...
                    paths.append(GlobalPath(path_id=pid, robot_key=robot_key,
                                            waypoints=final_wps, pivot=0.0))
        ...
        if next_dir in ('diag', 'any'):
            for d in np.arange(gs, diag_max, gs):
                nx = round(curr[0] + d, 2)
                ny = round(curr[1] + d, 2)
                if nx <= cfg.field["x_max"] and ny <= cfg.field["y_max"]:
                    nxt = (nx, ny)
                    if _path_valid([curr, nxt], cfg):
...
def validate_path_set(
    ...
    for paths in path_sets.values():
        for path in paths:
            ...
            if path.valid:
                path.footprint  = _cells_within_radius(path.waypoints, FOOTPRINT_RADIUS,  all_cells, x_min, y_min, gs)
                path.separation = _cells_within_radius(path.waypoints, SEPARATION_RADIUS, all_cells, x_min, y_min, gs)
                path.coverage   = _cells_within_radius(path.waypoints, COVER_RADIUS,      all_cells, x_min, y_min, gs)
                path.collinear  = _cells_within_radius(path.waypoints, COLLINEAR_RADIUS,  all_cells, x_min, y_min, gs)
            else:
...
def _build_wall_cells_from_sets(
    ...
    for cell in all_cells:
        cx = (cell[0] + 0.5)   # cell index, not metric — heuristic only
        cy = (cell[1] + 0.5)
...
# ── Interpolation helper (used by visualizer) ─────────────────────────────────

INTERP_MAX_DIST = 0.5   # m

def _interpolate_path(waypoints: List[Waypoint]) -> List[Waypoint]:
    """
    ...
    for i in range(len(waypoints) - 1):
        ...
        length = _math.hypot(p2[0] - p1[0], p2[1] - p1[1])
        n = max(1, _math.ceil(length / INTERP_MAX_DIST))
        for k in range(n):
            t = k / n
            result.append((
                round(p1[0] + t * (p2[0] - p1[0]), 2),
