...
* **동적 경로 탐색 (Forward-only BFS):** 0.5m 해상도의 그리드 상에서 목적지를 향해 전진하는 방향으로 최대 6개의 웨이포인트를 가지는 수천 개의 후보 경로를 자동 생성합니다.
* **물리적 제약과 커버리지의 분리:** 로봇의 주행 경로 유효성(Validation) 검증 시에는 벽 및 장애물로부터 0.3m의 안전거리(Clearance)를 엄격히 적용합니다. 반면, 가시 영역(Coverage) 계산 시에는 Clearance를 무시하여 실제 벽면 끝까지 도달하는 것으로 평가합니다.
* **다중 로봇 공간 분산 (Spatial Distribution):** 최대 편차 방식(Max Deviation)을 적용하여 로봇들이 중앙에 밀집하지 않고 맵의 외곽 및 넓은 구역으로 퍼지도록 유도합니다.
* **병렬 주행 제어 및 선간 이격:** 로봇 간 최소 안전 이격 거리(1.2m)를 강제하고, 경로 선분에 반발장(Separation Radius 0.6m)을 적용하여 좁은 통로에서 로봇들이 겹치는 현상(Funneling)을 방지합니다. 단, 출발지 및 도착지 반경 1.5m 이내는 수렴/발산 구역으로 간주하여 해당 페널티를 면제합니다.
...
| 항목 | 설명 |
| --- | --- |
| `starts` | 로봇별 출발 좌표 (예: `robot_01`, `sim_02`, `sim_03`) |
| `end` | 공통 도착 좌표 |
| `lobby` | 맵의 주 공간(로비) 물리적 경계 좌표 |
| `corridor_1` | 맵의 추가 공간(복도) 물리적 경계 좌표 (선택 사항) |
| `waypoint_sampling` | 경유지 격자 샘플링 범위 및 간격 (주로 BFS 탐색 그리드 생성에 활용) |
...
| 파라미터 | 기본값 | 설명 |
| --- | --- | --- |
| `frame_id` | `"map"` | Path 메시지 헤더의 frame_id |
...
ROS2 환경을 구동하지 않고 단독으로 경로 생성 로직을 테스트하고 결과를 이미지로 확인할 수 있습니다. 시각화 결과물은 패키지 루트 디렉토리에 PNG 파일로 저장됩니다.
