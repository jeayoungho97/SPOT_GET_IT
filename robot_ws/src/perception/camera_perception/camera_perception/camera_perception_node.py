...
class CameraPerceptionNode(Node):
    ...
    def __init__(self):
        ...
        self.smooth_frames  = self.get_parameter('depth_smooth_frames').value
    ...
    def rgb_callback(self, msg: Image):
        ...
        # 거리 추정
        distance = self._estimate_distance(frame, bbox)

        self.get_logger().info(
            f'탐지 | bbox: [{bbox[0]:.3f}, {bbox[1]:.3f}, {bbox[2]:.3f}, {bbox[3]:.3f}] '
            f'conf: {best["conf"]:.3f} dist: {distance:.2f}m'
        )
        ...
        if self.show_preview:
            ...
            cv2.putText(vis, f'conf:{best["conf"]:.2f} dist:{distance:.1f}m',
    ...
    def _estimate_distance(self, frame: np.ndarray, bbox: list) -> float:
        """bbox 하단 중심 ROI 기반 Depth 거리 추정 (N-frame smoothing)"""
        ...
        cx = np.clip(cx, r, w - r - 1)
        cy = np.clip(cy, r, h - r - 1)

        roi = self.depth_frame[cy - r:cy + r + 1, cx - r:cx + r + 1]
        valid = roi[roi > 0]
        if len(valid) == 0:
            return 0.0
        ...
        # N-frame smoothing
        self.distance_buf.append(distance_m)
        if len(self.distance_buf) > self.smooth_frames:
            self.distance_buf.pop(0)

        return float(np.mean(self.distance_buf))
